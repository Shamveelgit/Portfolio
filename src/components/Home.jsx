...
export default function Main(props) {
  ...
  const [isVisible,homeRef] = useInterSectionObserver({threshold : 1},true)
  const [checkVisibile,sectionRef] = useInterSectionObserver({threshold : .1},true)
  ...
  const buttonStyle = "border-solid border-2 border-blue-700 text-black hover:bg-blue-600 hover:text-white hover:border-blue-600 transition-all ease-linear duration-300";

  return (
    <main ref={homeRef} className="select-none w-full min-h-screen " id='home' >
      <section ref={sectionRef} className={`w-full min-h-screen   items-center justify-center flex-wrap relative ${isVisible ? "flex" : "hidden"}`}>
        <aside className='md:w-1/2' >
          <div className=" flex items-center justify-center p-4 h-4/5">
        ...
        <span id='triangle' className=' max-xl:hidden 2xl:hidden translate-y-[-10%]' style={assets.triangle}></span>
        </aside>
      </section> 
    </main>
  );
